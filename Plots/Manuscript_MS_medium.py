...
for i, ax in enumerate(axes.flat):
    ...
    for j, info in enumerate(plot_info):
        # print(j)
        if i==0:
            ax.plot(x, [y1, y2, y3,y4,y8][j], label=info['label'],marker="^", color=info['color'] )
        if i==1:
            ax.plot(x, [a1, a2, a3,a4,a8][j], label=info['label'],marker="^", color=info['color'])
        if i==2:
            ax.plot(x, [b1, b2, b3,b4,b8][j], label=info['label'], marker="^",color=info['color'])
        if i==3:
            ax.plot(x, [c1, c2, c3,c4,c8][j], label=info['label'], marker="^",color=info['color'])
        if i==4:
            ax.plot(x, [d1, d2, d3,d4,d8][j], label=info['label'], marker="^",color=info['color'])
        if i==5:
            ax.plot(x, [e1, e2, e3,e4,e8][j], label=info['label'],marker="^", color=info['color'])
    if i==0:
