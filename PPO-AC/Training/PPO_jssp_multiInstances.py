...
from AC_DNN_0 import ActorCritic
...
def main():
    ...
    dataLoaded = np.load('generatedData' + str(configs.n_j) + '_' + str(configs.n_m) + '_Seed' + str(configs.np_seed_validation) + '.npy')
    vali_data = []
    ...
    for i_update in range(configs.max_updates):
        ...
        #Reset the environment, start a new play from the beginning
        for i, env in enumerate(envs):
           
            adj, fea, candidate, mask = env.reset((data_generator()[0],data_generator()[1]))
            adj_envs.append(adj)
            fea_envs.append(fea)
            candidate_envs.append(candidate)
            mask_envs.append(mask)
            ep_rewards[i] = - env.initQuality
        # rollout the env, explore the env to take decision
        while True:
            ...
            for i in range(configs.num_envs):
                ...
                mask_envs.append(mask)
                ep_rewards[i] += reward
                #print("ep_reward[i]",ep_rewards[i])
        ...
        #print("loss, v_loss after update",loss, v_loss) 
        ...
        if i_update+1==10000:
           ...
           file_temp.write("data:"+str([envs[0].dur,envs[0].m])+"\n")
           file_temp.write("temp1:"+str(list(envs[0].temp1))+ "\n")
           file_temp.write("mchsstarttimes:"+str(list(envs[0].mchsStartTimes))+"\n")
           file_temp.write("opIdsonmchs:"+str(list(envs[0].opIDsOnMchs))+"\n")
           #np.array([env.dur,env.m]),,env.opIDsOnMchs
        # log results
        print('Episode {}\t Last reward: {:.2f}\t Mean_Vloss: {:.8f}'.format(
            i_update, mean_rewards_all_env, v_loss))
        
        # score_m.append(mean_rewards_all_env)
        # file_writing_obj2 = open(
        #         './' + 'CompletionTime_' + str(configs.n_j) + '_' + str(configs.n_m) + '_' + str(configs.low) + '_' + str(configs.high) + '.txt', 'w')
        # file_writing_obj2.write(str(score_m))
        # print("the makespan list is: ",score_m )        
...
if __name__ == '__main__':
    ...
    main()
    total2 = time.time()
    print("total time:", total2 - total1)