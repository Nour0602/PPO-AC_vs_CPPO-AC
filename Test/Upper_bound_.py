...
# load optimal policy
path = "./"+inst+"/"+alpha+'/{}.pth'.format(str(10) + '_' + str(110) + '_' + str(LOW) + '_' + str(HIGH))
print("path is", path)
...
for simulation in range(N_sim):
    ...
    executed_ops=[]
    print("initial solution is:", np.max(dico_end[0]))

    hour=1
    # tresh1=int(0.3*dico_ms[0])
    # tresh2=int(0.5*dico_ms[0])
    # tresh= int(random.uniform(tresh1,tresh2))
    tresh=int(0.01*dico_ms[0])
    print(tresh)
    while hour<=tresh:
        # print(hour is:", hour)
        executed_ops=[]
        ...
        if  hour==tresh:
        # hour==100 or hour==100 or hour==200 or hour==500 or hour==700 or hour==900 :      
            add_poisson=2
        # if hour==250:
        #     add_poisson=1      
        else:
            add_poisson=0 
        # add_poisson=np.random.poisson(1)                
        ...
        if add_poisson!=0:
            ...
            # print("executed op are:", executed_ops)
            # print("len(dico_eliminated[hour])",dico_eliminated[hour])
            ...
            while True:
                        ...
                        if done:
                            ...
                            break
            # print("hour, end is:", hour,end)           
            # print("env.temp1:", env.temp1)
            # print("DEV IS/", DEV[-1])
            # print("DEV IS/", DEV[-1][:,-1].sum())
            ending=time.time()
            # print("compute time",ending-starting )
            dico_compute_time[hour]=ending-starting
                        
            sum1 = DEV[-1][:,-1].sum() 
            # sum1=np.max(DEV[-1])
            # print("max deviation:", sum1, ep_reward)
            # stab=0
            # for i in range (N_JOBS_N):
            #     if env.temp1[i][N_MACHINES_N-1]!=data[4][i][N_MACHINES_N-1]:
            #         stab+=1
            # print(env.temp1,data[4], stab)
            #  To define the start-operation matrix since we have the matrix of start times according to machines
...
scenarios_file=open(location+"upper_bound_data.pkl", "wb")
