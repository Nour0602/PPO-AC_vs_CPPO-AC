...
import numpy as np
from obj import *
from IBM_sp_sol import optimize_and_visualize
...
# Executed tasks constraint
    ...
    # print("end is:", end[prev])
    # print("start per task is:", start_dc[prev])
    # print("eliminated cp is:", eliminated_cp)
    ...
    for job in instance.jobs:
        for task in job.tasks:
            if task.name not in eliminated_cp:
                ...
                processed.append((task,job.id,start_dc[prev][job.id-1][task.order-1], task.name))
    # print("start_dc[prev]", start_dc[prev]) 
    # print("end[prev]:", end[prev])
    ...
    for simulation in simulations:
        ...
        #Define the hour to make the comparison
            if hour==0:
                ...
                # print("Initial Ms:", MS)
                # print("dur,seq, eliminated sol_seq,start_cp,end_cp,MS,DE", dur[0],seq[0],eliminated ,sol_seq,start_cp,end_cp,MS,DE)
                ...
                if simulations[simulation]["added"][hour]!=0:
                    ...
                    cp_time=simulations[simulation]["compute_time"][hour]
                    # print("dur[hour]",dur[hour], seq[hour],eliminated[hour])


                    reader = reading(hour,prev,dur , seq, start_dc, end, eliminated)
                    # print("Initial Ms",np.max(end[0]))
                    ...
                    t2=time.time()
                    time_to=t1-t2
                    # print("scheduled task:", scheduled_tasks[0])
                    job_ref=[]
                    job_endg=[]
                    deviations = []
                    makespan = 0
                    ...
                    for job in instance.jobs:
                        # if job.id not in eliminated_jobs:
                        if job.ref!=0:
                            L=[t[2]+t[0].length for t in scheduled_tasks if t[0].job == job]
                            # job_end = max(t[2]+t[0].length for t in scheduled_tasks if t[0].job == job)
                            job_end= max(L)
                            # print("job id, end:",job.id, job_end,job.ref)
                    ...
                    # print("Total deviation:", sum(deviations))
                    ...
                    # print("MS,maximum dev:", MS,max(deviations))
                    # DF, data, eliminated_cp,sol_seq,start_cp,end_cp,MS,DE,time_to=optimize_and_visualize( hour,prev,dur , seq, start_dc, end,eliminated,1000,1)
