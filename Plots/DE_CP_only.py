...
# data_y4 = [pd.read_excel(file)['Mean_deviation'].values[5] for file in files]
# data_y4 = [round(float(data),2) for data in data_y4]
# data_y5 = [pd.read_excel(file)['Mean_deviation'].values[6] for file in files]
# data_y5 = [round(float(data),2) for data in data_y5]
data_y6 = [pd.read_excel(file)['Mean_deviation'].values[1] for file in files]
...
for i, ax in enumerate(axes.flat):
    ...
    for j, info in enumerate(plot_info):
        # print(j)
        if i==0:
            ax.plot(x, [y1, y2][j], label=info['label'],marker="^", color=info['color'] )
        if i==1:
            ax.plot(x, [a1, a2][j], label=info['label'],marker="^", color=info['color'])
        if i==2:
            ax.plot(x, [b1, b2][j], label=info['label'], marker="^",color=info['color'])
        if i==3:
            ax.plot(x, [c1, c2][j], label=info['label'], marker="^",color=info['color'])
        if i==4:
            ax.plot(x, [d1, d2][j], label=info['label'], marker="^",color=info['color'])
        if i==5:
            ax.plot(x, [e1, e2][j], label=info['label'],marker="^", color=info['color'])
    if i==0:
        ax.set_title(f'Inst. 10x10')
    if i==1:
        ax.set_title(f'Inst. 15x15') 
