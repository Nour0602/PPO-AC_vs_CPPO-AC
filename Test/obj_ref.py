...
class Task:
    def __init__(self, name, order, machine, length):
        self.name = name
        self.order=order #task ordr starts at 1
...
class reading:
    ...
    def __init__(self, dur, seq):      
    #  2- Prepare jobs, tasks and machines
        jobs = []
        machines = []
        # task_counter = 0
        num_jobs = len(dur)
        num_machines = len(seq)
...
class Solution:
    ...
    def add(self, task, start, end, machine):
        self.solution[task] = (start, end, machine)
        self.tasks.append(task)
